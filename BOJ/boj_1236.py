...
for i in range(n):
    for j in range(m):
        if sung[i][j]=="X":
            check.append([i,j])
n_list=[int(x) for x in range(0,n)]
m_list=[int(x) for x in range(0,m)]


for i in range(len(check)):
    if check[i][0] in n_list:
        try:
            n_list.remove(check[i][0])
        except:
            pass
    if check[i][1] in m_list:
        try:
            m_list.remove(check[i][1])
        except:
            pass

print(max(len(m_list),len(n_list)))