"""
문제 : N(2≤N≤10,000)개의 섬으로 이루어진 나라가 있다.
...
입력 : 첫째 줄에 N, M(1≤M≤100,000)이 주어진다.
...
def bfs(weight):
    visited=[False]*(n+1)
    # node 가 1부터 시작하므로 n+1로 만들어준것이다. 그래야 n+1의 인덱스가 n 이다.
    need_visit=[]
    ...
    while need_visit:
        ...
        if not visited[node]:
            visited[node] = True
            for arrive,wgt in island[node]:
                if wgt>=weight:
                    need_visit.append(arrive)
                    if arrive==end:
                        return True
    return False
